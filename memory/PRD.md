...
- 2026-09: NEW Admin delete-user UI — Danger Zone (two-step confirm) in the user-detail dialog calls DELETE /api/admin/users/{id} (backend endpoint pre-existed; blocks admin/self, cascades stakes+txns+audit). ADDED instant session kick: backend emits WS {type:'force_logout'} to the deleted user; AuthContext handles it (clears token+vault, toast, redirect to /). Backend tested 10/10 pass.
- 2026-09: THREE enhancements (all backend-tested 17/17). (1) Withdrawal ADDRESS BOOK: user doc saved_addresses[] + GET/POST/DELETE /api/withdraw-addresses (label+XRP-validated address+optional numeric tag, dedupe); Withdraw page shows saved chips (click to fill, x to delete) + "Save this address" link. (2) Restake PREVIEW: extracted pure restake_projection(); POST /api/reinvest/preview {stake_id} returns projected new_principal/new_matures_at/profit_at_maturity/total_at_maturity WITHOUT mutating; ReinvestDialog fetches it and shows exact new maturity date + payout before confirm. (3) Admin ACTIVITY FEED: GET /api/admin/activity merges deposits+withdrawals+audit actions+signups newest-first; new Admin > Activity tab (auto-refreshes via xp-refresh WS).
