        ...
        {step === 1 && (
          <motion.div key="reveal" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xl">
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={copyPhrase} data-testid="copy-phrase-button" className="flex-1 flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 text-sm font-medium py-3 rounded-xl transition-colors">
                {copied ? <Check size={16} className="text-emerald-600" /> : <Copy size={16} />} {copied ? "Copied" : "Copy phrase"}
              </button>
              <button onClick={() => setRevealed(!revealed)} data-testid="toggle-phrase-visibility" className="px-4 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 rounded-xl transition-colors">
